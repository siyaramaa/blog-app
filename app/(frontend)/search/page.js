...
const BlogCard = dynamic(() => import("@/components/BlogCard"), {
  ...
  loading: () => (
    <div
      role="status"
      className="animate-pulse container max-w-xl h-[30vh]  p-5 mx-auto rounded-lg shadow-lg dark:bg-gray-800"
    >
...
//By Category
const getBlogByTag = async (searchQuery) => {
  const query = `*[_type == 'post' && "${searchQuery}" in categories[]->title]{
                title,            
                    slug,
                _createdAt,
                mainImage,
...
export default async function SearchPage({ params, searchParams }) {
    const query = searchParams.q || searchParams.byTag;
    const method = searchParams.byTag ? "tagMethod" : 'queryMethod';
    let searchedBlogs;
    if(method == "tagMethod"){
         searchedBlogs = await getBlogByTag(query);

    }else{
         searchedBlogs = await getBlogByQuery(query); 
    }

  return (
    <div className="min-h-screen">
      {
        method != "tagMethod" && searchedBlogs?.length != 0 ? (

          <p className="border-t border-b border-gray-200 dark:border-gray-700 p-3">Search results for {query}
            </p>
          ) :           <p className="border-t border-b border-gray-200 dark:border-gray-700 p-3">Posts for {query}
          </p>
      }
          ...
          <div className="container flex items-center min-h-screen px-6 py-12 mx-auto">
            <div className="flex flex-col items-center max-w-sm mx-auto text-center">
              ...
              <p className="mt-4 text-gray-500 dark:text-gray-400">
                The blog you are looking for doesn&apos;t exist. Here are some
                helpful links:
              </p>
